...
async def main(args: argparse.Namespace) -> None:
    ...
    async with WirelessGoPro(args.identifier1, enable_wifi=False) as gopro1, \
        ...
        while True:
            user_input = await ainput("Enter 1 to start camera 1, 2 to start camera 2, or q to quit: ")
            console.print(f"[yellow]User input: {user_input}")
            if user_input.strip() == '1':
                console.print("[yellow]Starting camera 1")
                await start_livestream(gopro1)
            elif user_input.strip() == '2':
                console.print("[yellow]Starting camera 2")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("url2", type=str, help="RTMP server URL to stream to for the second GoPro.", nargs='?', default="rtmp://192.168.0.246:1935/live/mystream")
